    ...
    <div className="min-h-screen bg-[#F5F5FA] font-sans pb-28">
      ...
      <div className="px-6 pt-12 pb-6 flex justify-between items-center bg-white sticky top-0 z-10 shadow-sm md:static md:bg-transparent md:shadow-none">
        <div className="flex items-center gap-3">
          ...
          <div className="w-12 h-12 rounded-full bg-yellow-300 overflow-hidden border-2 border-white shadow-sm flex items-center justify-center">
            {/* ใส่รูปโปรไฟล์ถ้ามี หรือใช้ Emoji แทน */}
        ...
        <div className="flex gap-3">
           {/* Fake Icons */}
          <button className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-600 shadow-sm">⚙️</button>
      ...
      <div className="max-w-3xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          ...
          <div className="bg-[#EBEBFF] p-5 rounded-[2rem] flex flex-col justify-between h-40 relative group hover:shadow-md transition-all">
            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-lg mb-2">📅</div>
            <div>
          ...
          {/* Card 4: Completed */}
          <div className="bg-[#F2F0FF] p-5 rounded-[2rem] flex flex-col justify-between h-40 relative group hover:shadow-md transition-all">
             <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-lg mb-2">✅</div>
            <div>
        ...
        <div className="flex gap-3 mb-6 overflow-x-auto pb-2 scrollbar-hide">
          <button 
            ...
            className={`px-6 py-3 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${
              filter === "all" ? "bg-[#1C1D22] text-white shadow-lg" : "bg-white text-slate-500"
            }`}
          >
            All Tasks
          </button>
          <button 
            ...
            className={`px-6 py-3 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${
              filter === "pending" ? "bg-[#1C1D22] text-white shadow-lg" : "bg-white text-slate-500"
            }`}
          >
            In Progress
          </button>
          <button 
            ...
            className={`px-6 py-3 rounded-full text-sm font-semibold whitespace-nowrap transition-all ${
              filter === "completed" ? "bg-[#1C1D22] text-white shadow-lg" : "bg-white text-slate-500"
            }`}
          >
            Completed
          </button>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end mb-2">
            ...
            <span className="text-slate-400 text-xs">Today, {new Date().getDate()} {new Date().toLocaleString('en-US', { month: 'short' })}</span>
          </div>
          ...
          {loading ? (
            ...
            filteredTasks.map((task) => {
              ...
              return (
                <div key={task.id} className="bg-white p-5 rounded-[2rem] shadow-sm flex flex-col gap-3 relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Link href={`/updatetask/${task.id}`} className="text-slate-300 hover:text-blue-500">✏️</Link>
                      <button onClick={() => handleDelete(task.id)} className="text-slate-300 hover:text-red-500">🗑️</button>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 items-center">
                    <div className="flex-1">
                      <h4 className={`text-lg font-bold mb-1 ${task.is_completed ? "text-slate-400 line-through" : "text-slate-800"}`}>
                        {task.title}
                      </h4>
                      <p className="text-sm text-slate-500 line-clamp-1">{task.detail}</p>
                    </div>
                    ...
                    {imgSrc ? (
                      <div className="w-12 h-12 rounded-2xl overflow-hidden bg-slate-100 flex-shrink-0 border border-slate-100">
                        <Image src={imgSrc} width={48} height={48} alt="task" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                  ...
                  <div className="flex gap-2 mt-2">
                    <span className={`px-4 py-2 rounded-full text-xs font-bold ${
                      task.is_completed ? "bg-green-100 text-green-700" : "bg-slate-900 text-white"
                    }`}>
                      {task.is_completed ? "Done" : "Today"}
                    </span>
                    {/* Fake duration tag for style */}
                    <span className="px-4 py-2 rounded-full text-xs font-bold bg-slate-100 text-slate-500">
                      1h 30m
                    </span>
                  </div>
                </div>
      ...
      {/* =============================================
       ...
       <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-100 px-6 py-4 flex justify-center items-center z-50 text-slate-400 pb-8 md:pb-4 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
         ...
         <Link href="/addtask" className="w-14 h-14 bg-[#4B4CED] rounded-full flex-col flex items-center justify-center text-white shadow-xl shadow-indigo-300 -mt-8 hover:scale-105 transition-transform border-4 border-[#F5F5FA]">
